...
int main(int argc, char const *argv[]) {
  if (argc != 4){
    std::cout << "\E[31mUssage: ./TSPFirst fileName numberOfProblemOfTheObject numberOfLocalOptimizationSolutions\e[0m" << std::endl;
    return -1;
  }
  srand(time(NULL));
  ...
  for (unsigned int i = 0; i < nRandomSearchSolutions; i++) {
    ...
    TSP::FirstNeighExplorator b(nLocalOptimizationSolutions);
    TSP::LocalSearch l(b);
    l.search(solution);
  ...
  for (auto it = std::begin(localOptimizationDistances) ; it + randomSearchDistances.size() <= std::end(localOptimizationDistances); it += aumento){
    ...
    if (randomSearchDistances[k] > randomSearchDistances[k+1]){
  ...
  for (unsigned int i = 0 ; i < localOptimizationDistances.size() ; i ++){
    ...
    localOptimizationSolutionsFile << localOptimizationDistances[i] << " " << tamLocalOptimizationVector[i] << std::endl;
